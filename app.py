...
def add_csv():
    ...
    with open('suggested_books.csv') as csvfile:
        ...
        for row in data:
            ...
            if book_in_db == None:
                new_book = Book(
                    title=row[0], author=row[1], published_date=date, price=price)
    ...
    while app_running:
        ...
        if choice == '1':
            ...
            new_book = Book(title=title, author=author,
                            published_date=published, price=price)
            session.add(new_book)
            session.commit()
            print('Book added!')
            time.sleep(1.5)
            ...
            if menu_choice == '1':
                ...
                the_book.published_date = edit_check(
                    'Published Date', the_book.published_date)
            ...
            print(f'''
                  \n***** BOOK ANALYSIS *****
                  \rOledset Book: {oldest_book}
                  \rNewest Book: {newest_book}
                  \rTotal Book: {total_books}
